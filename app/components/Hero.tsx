...
export default function Hero({ isDarkMode, setIsDarkMode }: HeroProps) {
  return (
    <section data-testid="hero-section" className="w-full pt-16 px-4 sm:px-6 md:px-8 lg:px-16 flex flex-col lg:flex-row items-center justify-center gap-6">
      ...
      >
        <h2 data-testid="main-hero-title" className="text-3xl sm:text-4xl font-bold font-playwrite">Women&apos;s Fashion</h2>
        <p data-testid="main-hero-description" className="mt-2 text-sm sm:text-base">
          Discover the latest trends in women&apos;s fashion.
        </p>
        <button data-testid="main-hero-cta" className="mt-4 bg-[#780000] text-white px-4 sm:px-5 py-2 rounded-full text-sm sm:text-base cursor-pointer hover:translate-y-1 hover:shadow-md">
          Shop now
        </button>
      </div>
      ...
      <div data-testid="hero-grid" className="w-full lg:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-4 text-[#0d1b2a]">
        {heroPic.map(({ id, bgImg, title, items }) => (
          ...
          >
            <h2 data-testid={`grid-title-${id}`} className="text-lg sm:text-xl font-semibold font-playwrite">{title}</h2>
            <p data-testid={`grid-description-${id}`} className="text-xs sm:text-sm">{items}</p>
            <button data-testid={`grid-cta-${id}`} className="mt-2 bg-[#780000] text-white px-3 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm cursor-pointer hover:translate-y-1 hover:shadow-md">
              Shop now
            </button>
          </div>
        ))}
